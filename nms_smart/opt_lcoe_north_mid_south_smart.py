...
#%% Competitive design
    ...
    # Adjust order of substations
    Subs_x = [Subs_x[2]] + [Subs_x[1]] + [Subs_x[0]]
    Subs_y = [Subs_y[2]] + [Subs_y[1]] + [Subs_y[0]]
    # ----------------------------------------
    # 1.) Pre-Optimize South Zone
    #
    # Initital layout
    ...
    tf = TopFarmProblem(
            ...
            driver = EasySGDDriver(maxiter=5, learning_rate=windTurbines.diameter(), max_time=1008000, gamma_min_factor=0.1, speedupSGD=True, sgd_thresh=0.12),
            plot_comp = XYPlotCompBathym(save_plot_per_iteration=True, plot_initial=False, memory=0, X=X_utm, Y=Y_utm, Z=Z, Sx=Sx, Sy=Sy, cables=cables, metrics_recorder=metrics_recorder, Xn=xn, Yn=yn, b=boundplot, nr=1),
            )
    ...
    x_south = state['x']
    y_south = state['y']
    cable_cost_south = metrics_recorder['cable_cost'][-1]
    cable_u_south = metrics_recorder['cable_u'][-1]
    cable_v_south = metrics_recorder['cable_v'][-1]
    cable_type_south = metrics_recorder['cable_type'][-1]
    mp_cost_south = metrics_recorder['mp_cost'][-1]
    
    # ----------------------------------------
    # 2.) Pre-Optimize Mid Zone
    #
    # Initital layout
    ...
    xn = state['x']
    yn = state['y']
    cable_cost_n[0] = cable_cost_south
    cable_u_n[0] =cable_u_south
    cable_v_n[0] = cable_v_south
    cable_type_n[0] = cable_type_south
    mp_cost_n[0] = mp_cost_south
    ...
    tf = TopFarmProblem(
            ...
            constraints = DistanceConstraintAggregation([SpacingConstraint(min_spacing_m), constraint_comp],tur_nr, min_spacing_m, windTurbines), 
            driver = EasySGDDriver(maxiter=5, learning_rate=windTurbines.diameter(), max_time=1008000, gamma_min_factor=0.1, speedupSGD=True, sgd_thresh=0.12),
            plot_comp = XYPlotCompBathym(save_plot_per_iteration=True, plot_initial=False, memory=0, X=X_utm, Y=Y_utm, Z=Z, Sx=Subs_x[:2], Sy=Subs_y[:2], cables=cables, metrics_recorder=metrics_recorder, Xn=xn, Yn=yn, b=boundplot, nr=2),
            )
    ...
    metrics_recorder["x_final"].append(state['x'].tolist())
    metrics_recorder["y_final"].append(state['y'].tolist())
    cable_cost_mid = metrics_recorder['cable_cost'][-1]
    cable_u_mid =[x-1 for x in metrics_recorder["cable_u"][-1][tur_nr:]]
    cable_v_mid = [y-1 for y in metrics_recorder["cable_v"][-1][tur_nr:]]
    cable_type_mid = metrics_recorder['cable_type'][-1][tur_nr:]
    mp_cost_mid = metrics_recorder['mp_cost'][-1]

    # ----------------------------------------
    # 3.) Optimize North Zone
    #
    # Initital layout
    ...
    cable_cost_n[1] = cable_cost_mid
    cable_u_n[1] = cable_u_mid
    cable_v_n[1] = cable_v_mid
    cable_type_n[1] = cable_type_mid
    mp_cost_n[1] = mp_cost_mid
    ...
    tf = TopFarmProblem(
            ...
            constraints = DistanceConstraintAggregation([SpacingConstraint(min_spacing_m), constraint_comp],tur_nr, min_spacing_m, windTurbines), 
            driver = EasySGDDriver(maxiter=5, learning_rate=windTurbines.diameter(), max_time=1008000, gamma_min_factor=0.1, speedupSGD=True, sgd_thresh=0.12),
            plot_comp = XYPlotCompBathym(save_plot_per_iteration=True, plot_initial=False, memory=0, X=X_utm, Y=Y_utm, Z=Z, Sx=Subs_x, Sy=Subs_y, cables=cables, metrics_recorder=metrics_recorder, Xn=xn, Yn=yn, b=boundplot, nr=3),
            )
    ...
    metrics_recorder["x_final"].append(state['x'].tolist())
    metrics_recorder["y_final"].append(state['y'].tolist())
    x_north_final = state['x']
    y_north_final = state['y']
    cable_cost_north = metrics_recorder['cable_cost'][-1]
    cable_u_north =[x-1 for x in metrics_recorder["cable_u"][-1][tur_nr:]]
    cable_v_north = [y-1 for y in metrics_recorder["cable_v"][-1][tur_nr:]]
    cable_type_north = metrics_recorder['cable_type'][-1][tur_nr:]
    mp_cost_north = metrics_recorder['mp_cost'][-1]
    
    # ----------------------------------------
    # 4.) Optimize Mid Zone
    Subs_x = [Subs_x[2]] + [Subs_x[0]] + [Subs_x[1]]
    Subs_y = [Subs_y[2]] + [Subs_y[0]] + [Subs_y[1]]
    #
    # Initital layout
    x0 = np.array(points_m).T[0]
    y0 = np.array(points_m).T[1]
    ...
    Sy = [Subs_y[1]]
    nf = True      # Neighbour wind farm coordinates = results from foregoing optimization
    xn = np.concatenate([x_south, state['x']])
    yn = np.concatenate([y_south, state['y']])
    cable_cost_n[0] = cable_cost_mid
    cable_cost_n[1] = cable_cost_north
    cable_u_n[0] = cable_u_mid
    cable_u_n[1] = cable_u_north
    cable_v_n[0] = cable_v_mid
    cable_v_n[1] = cable_v_north
    cable_type_n[0] = cable_type_mid
    cable_type_n[1] = cable_type_north
    mp_cost_n[0] = mp_cost_mid
    mp_cost_n[1] = mp_cost_north
    ...
    tf = TopFarmProblem(
            ...
            constraints = DistanceConstraintAggregation([SpacingConstraint(min_spacing_m), constraint_comp],tur_nr, min_spacing_m, windTurbines), 
            driver = EasySGDDriver(maxiter=5, learning_rate=windTurbines.diameter(), max_time=1008000, gamma_min_factor=0.1, speedupSGD=True, sgd_thresh=0.12),
            plot_comp = XYPlotCompBathym(save_plot_per_iteration=True, plot_initial=False, memory=0, X=X_utm, Y=Y_utm, Z=Z, Sx=Subs_x, Sy=Subs_y, cables=cables, metrics_recorder=metrics_recorder, Xn=xn, Yn=yn, b=boundplot, nr=4),
            )
    ...
    # ----------------------------------------
    # 5.) Optimize South Zone
    Subs_x = [Subs_x[0]] + [Subs_x[1]] + [Subs_x[2]]
    Subs_y = [Subs_y[0]] + [Subs_y[1]] + [Subs_y[2]]
    #
    # Initital layout
    x0 = np.array(points_s).T[0]
    y0 = np.array(points_s).T[1]
    ...
    Sy = [Subs_y[2]]
    nf = True      # Neighbour wind farm coordinates = results from foregoing optimization
    xn = np.concatenate([x_north_final, state['x']])
    yn = np.concatenate([y_north_final, state['y']])
    cable_cost_n[0] = cable_cost_north
    cable_cost_n[1] = cable_cost_mid
    cable_u_n[0] = cable_u_north
    cable_u_n[1] = cable_u_mid
    cable_v_n[0] = cable_v_north
    cable_v_n[1] = cable_v_mid
    cable_type_n[0] = cable_type_north
    cable_type_n[1] = cable_type_mid
    mp_cost_n[0] = mp_cost_north
    mp_cost_n[1] = mp_cost_mid
    ...
    tf = TopFarmProblem(
            ...
            constraints = DistanceConstraintAggregation([SpacingConstraint(min_spacing_m), constraint_comp],tur_nr, min_spacing_m, windTurbines), 
            driver = EasySGDDriver(maxiter=5, learning_rate=windTurbines.diameter(), max_time=1008000, gamma_min_factor=0.1, speedupSGD=True, sgd_thresh=0.12),
            plot_comp = XYPlotCompBathym(save_plot_per_iteration=True, plot_initial=False, memory=0, X=X_utm, Y=Y_utm, Z=Z, Sx=Subs_x, Sy=Subs_y, cables=cables, metrics_recorder=metrics_recorder, Xn=xn, Yn=yn, b=boundplot, nr=5),
            )
    ...
    # 5. Save to a file
    import pickle
    with open("metric_recorder_sequential_nms_smart.pkl", "wb") as file:
        pickle.dump({"metrics_recorder": metrics_recorder, "state": state}, file)