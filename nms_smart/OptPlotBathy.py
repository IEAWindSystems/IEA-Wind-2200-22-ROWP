...
class XYPlotCompBathym(ExplicitComponent):
    ...
    def __init__(self, memory=10, delay=0.001, plot_initial=True, plot_improvements_only=False, ax=None, legendloc=1, save_plot_per_iteration=False, X=None, Y=None, Z=None, Sx=None, Sy=None, cables=None, metrics_recorder=None, Xn=[], Yn=[], b=[], nr=1):
...
#         plt.plot(b[:, 0], b[:, 1], 'k')
    ...
    def set_title(self, cost0, cost):
        ...
        title += "\n Current LCOE = %.2f $/MWh (%+.2f%%)" % (cost, (cost - cost0) / cost0 * 100)
        #AEP
        title += " \n Current AEP = %.1f GWh (%+.2f%%)" % (self.metrics_recorder['aep'][-1]/1e3, (self.metrics_recorder['aep'][-1]/1e3 - self.metrics_recorder['aep'][0]/1e3) / (self.metrics_recorder['aep'][0]/1e3) * 100)
        #Cable Cost
        title += " \n Current Cab-Cost = %s $ (%+.2f%%)" % ("{:,.0f}".format(self.metrics_recorder['cable_cost'][-1]), (self.metrics_recorder['cable_cost'][-1] - self.metrics_recorder['cable_cost'][0]) / self.metrics_recorder['cable_cost'][0] * 100)
        #Monopile Cost
        title += " \n Current MP-Cost = %s $ (%+.2f%%)" % ("{:,.0f}".format(self.metrics_recorder['mp_cost'][-1]), (self.metrics_recorder['mp_cost'][-1] - self.metrics_recorder['mp_cost'][0]) / self.metrics_recorder['mp_cost'][0] * 100)
        ...
        if self.by_pass is False:
            ...
            if self.save_plot_per_iteration:
                ...
                if not os.path.exists('Figures'):
                    os.makedirs('Figures')
                plt.savefig('Figures/iteration_z' + str(self.nr) + '_%s.png' % self.counter)